(() => {
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/gallery.ts
  var StackGallery = class _StackGallery {
    ...
    constructor(container, galleryUID = 1) {
      ...
      this.bindClick();
      if (!window.stackGalleries) {
        window.stackGalleries = [];
      }
      window.stackGalleries.push(this);
    }
    loadItems(container) {
      this.items = [];
    ...
    open(index) {
      if (this.pswpInstance) {
        this.pswpInstance.close();
      }
      ...
      this.pswpInstance = new window.PhotoSwipe(
        pswp,
        ...
        this.items,
        {
          ...
          getThumbBoundsFn: (index2) => {
            const thumbnail = this.items[index2]?.el?.getElementsByTagName("img")[0];
            if (!thumbnail) {
              return { x: 0, y: 0, w: 0 };
            }
    ...
    destroy() {
      if (this.pswpInstance) {
        this.pswpInstance.close();
        this.pswpInstance = null;
      }
      this.clickHandlers.forEach((handler) => handler());
      this.clickHandlers = [];
      if (window.stackGalleries) {
        window.stackGalleries = window.stackGalleries.filter((gallery) => gallery !== this);
      }
    }
  };
  function handlePjaxEvents() {
    document.addEventListener("pjax:start", () => {
      if (window.stackGalleries) {
        window.stackGalleries.forEach((gallery) => gallery.destroy());
        window.stackGalleries = [];
      }
    });
  ...
  function initializeGalleries() {
    ...
    containers.forEach((container, index) => {
      new StackGallery(container, index + 1);
    });
  }
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/color.ts
  var colorsCache = {};
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/menu.ts
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/createElement.ts
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/colorScheme.ts
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/scrollspy.ts
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/smoothAnchors.ts
  ...
  // ns-hugo-imp:/root/hugo-stack/themes/hugo-theme-stack-3.30.0/assets/ts/search.tsx
  var tagsToReplace = {
    "&": "&amp;",
    "<": "&lt;",
    ">": "&gt;",
